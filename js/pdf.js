    ...
    try {
        ...
        const filas = data.data.map(item => {
            ...
            return {
                ...
                "Tiempo Muerto": item.tiempo_muerto || "0",
                "Días Estimados": item.cantidad || "",
                "Estado": obtenerEstadoExcel(item, fechaInicio, fechaFin)
            };
        });
        ...
        XLSX.utils.book_append_sheet(libro, hoja, "Carta Gantt");

        XLSX.writeFile(libro, "Carta_Gantt_LAGMET.xlsx");
...
function obtenerEstadoExcel(item, fechaInicio, fechaFin){
